...
[redis官网](https://github.com/redis/redis)

![image](../image.png)

## Redis简介

1. 高性能 : Redis是完全基于内存的开源非关系型数据库, 性能高于Memcached, 读写速度快于其他数据库。

2. 数据类型 : Redis支持五种数据类型, 包括字符串, 散列, 列表, 集合, 有序集合。

3. 持久性 : Redis支持两种持久化方式, 一种是RDB, 另一种是AOF。

4. 集群 : Redis支持主从复制, 实现读写分离, 提高Redis的可扩展性。

5. 事务 : Redis支持事务, 事务可以一次执行多个命令, 所有命令都成功或失败。

6. 发布/订阅 : Redis支持发布/订阅模式, 允许多个订阅者订阅同一个频道。

7. 脚本 : Redis支持脚本, 允许执行 Lua 脚本。

8. 高可用 : Redis支持主从复制, 自动故障转移, 提高Redis的可用性。

--- 

## 数据类型

Redis支持五种数据类型, 包括字符串, 散列, 列表, 集合, 有序集合。

### 字符串

字符串类型是Redis最基本的数据类型, 可以存储字符串, 整数或者浮点数。
...
列表类型是Redis最基本的有序集合, 可以存储多个值, 并且按照插入顺序排序。
...
有序集合类型是一种特殊的集合, 可以存储多个值, 并且按照插入顺序排序, 并且每个值都有一个分数。
...
![RDB流程图](https://code84.com/wp-content/uploads/2020/06/7d46e5911190d814a8b89cb0ce710527.png)
...
![AOF流程图](https://img2022.cnblogs.com/blog/2032648/202202/2032648-20220207170749957-1755223036.png)
...
- 开启事务 : `multi`, 存放进入服务器的事务队列, 类似SQL中的`start transaction`

- 执行命令 : `exec` , 类似SQL中的`commit`

- 取消事务: `discard`, 类似`rollback`
...
**Redis集群**: Redis集群是一种基于分布式存储的解决方案, 它提供了高可用性, 伸缩性, 数据共享性等特性。相当于我们通过多个Redis实例共享内存, 实现内存容量的扩充。

**通过hash分片进行数据分片**: Redis集群通过hash分片进行数据分片, 所有的键值对会根据key进行hash, 然后根据hash值将数据分片到不同的节点上。

存在的问题: 扩容的时候成本太高, 扩容需要迁移大量数据, 扩容之后需要重新分片, 这是一个非常耗时的操作。

解决方式: 一致性hash, 一致性hash算法可以将所有的节点映射到一个圆环上, 然后将数据映射到圆环上, 使得数据均匀分布。

**Redis真正采用的Hash分片的算法**
Hash槽分区算法: 

- 首先, 计算key的CRC32校验码, 然后对2^32取模, 得到一个0到2^32-1之间的整数值

- 然后, 根据这个整数值, 确定一个节点, 这个节点就是要存储这个key-value对的节点

- 最后, 节点负责处理这个key-value对
...
1. **读写分离**: 读多写少的业务场景, 可以通过Redis缓存来提升性能, 避免直接访问数据库, 提高响应速度。

2. **降低数据库压力**: 缓存可以降低数据库的压力, 避免大并发情况下数据库压力过大, 导致数据库宕机。

3. **降低网络延迟**: 缓存可以降低网络延迟, 避免大并发情况下, 数据库和应用服务器之间网络延迟过大, 导致请求超时。


### Redis的缓存淘汰机制

1. **随机淘汰(random eviction)**: 随机淘汰是指当缓存空间不足时, 随机删除一些缓存数据, 直到腾出足够的空间。

2. **先进先出淘汰(first in first out eviction)**: 先进先出淘汰是指当缓存空间不足时, 最先进入缓存的数据, 最先被淘汰。

3. **最近最少使用淘汰(least recently used eviction)**: 最近最少使用淘汰是指当缓存空间不足时, 最久没有被访问的数据, 最先被淘汰。

4. **最少使用次数淘汰(least frequently used eviction)**: 最少使用次数淘汰是指当缓存空间不足时, 最少被访问的次数最少的数据, 最先被淘汰。
...
1. 缓存预热:
    - 定期查询: 不存在这种问题
    - 实时生成: Redis作为数据库的缓存, 在刚开始的时候, 缓存是空的, 所以需要预先将数据加载到缓存中, 避免缓存穿透。
        我们可以通过离线脚本的方式加载一些热点数据到缓存中, 也可以通过定时任务的方式加载数据到缓存中。

2. 缓存穿透:
    > 当前的数据, 在Redis中没有, 在数据库中也没有, 这样大量的数据会直接查询数据库, 导致数据库压力过大, 甚至会导致宕机。

    - 解决方案: 缓存空数据, 避免查询数据库。

3. 缓存雪崩:

    > 短时间内Redis大部分的缓存失效, 导致大量请求直接查询数据库, 数据库压力过大, 甚至会导致宕机。

    **解决方式**

    - 加强监控报警
    - 不给key设置过期时间/给Redis设置随机的过期时间

4. 缓存击穿:

    > (cache break down), 特指针对热点的key缓存过期, 导致大量请求直接查询数据库, 数据库压力过大, 甚至会导致宕机。这种其实是缓存雪崩的一种特殊的情况。
...
> 当存在多个执行流的时候, 对于临界区的访问顺序和切换的时间是未定义的, 因此, 锁的引入可以帮助我们做好同步, 由于Redis是单线程的服务, 保证了原子性, 所以, 我们可以使用Redis的事务机制来实现分布式锁。

### 如何解决分布式锁的问题

对于C++, 我们可以使用`RAII`保证解锁, 对于分布式系统, 网络的稳定性是很大的变数, 这里我们应该使用设置过期时间(ttl), 保证不会出现死锁, 比如我们可以设置`1000 ps`作为锁的过期时间, 这样即使锁的持有者出现异常崩溃, 锁也会在`1000 ps`后自动释放, 其他进程可以获得锁。


### 通过lua脚本实现分布式锁(解决加锁解锁不是原子性的问题)

why ? redis保证了执行lua脚本的原子性, 所以, 我们可以使用lua脚本来实现分布式锁。(lua也是redis事务的替代方案)
...
- 如果过期时间过短, 业务可能还没有解决, 就提前释放了锁

- 如果过期时间过长, 可能影响分布式系统的性能

**动态续约的方案**(`看门狗`):

- 我们可以在初始的情况下, 我们可以设置一个较短的过期时间, 当时间快到了, 服务还没有结束, 我们就进行续约。

- 如果我们的服务崩溃了, 就没人续约了, 很自然的所就能在一个较短的时间内(续约长度)进行释放锁。

我们针对这种机制, 引入一个`看门狗`的线程, 定期检查过期时间的问题, 用于更新锁的过期时间。

 
### 如果Redis宕机了怎么办?

Redis集群的高可用性保证了Redis的高可用, 但是, 如果Redis宕机了, 我们需要考虑怎么办?

- 哨兵模式: 我们可以配置多个哨兵节点, 哨兵节点可以监控Redis集群的状态, 并进行故障转移。

- 集群模式: 我们可以配置多个Redis节点, 集群中的任意一个节点宕机, 集群仍然可以正常提供服务。

但是我们肯定还是推荐哨兵模式, 因为哨兵模式可以自动发现Redis节点的故障, 并进行故障转移, 而集群模式需要人工介入, 并且在分布式锁这种应用场景下, 数据量较小, 集群模式的性能瓶颈不在Redis, 所以, 我们还是推荐哨兵模式。

主节点挂了, 从节点成为主节点可能存在数据的缺失(数据同步延时)

**redlock算法**:

针对多个Redis节点, 我们都进行加锁, 某个节点加不上锁, 我们就认为redis挂了, 然后我们加锁下一个节点, 当超过一半的我们加锁成功了, 我们就认为加锁成功, 然后我们就可以进行操作。
当解锁的时候, 我们针对上述加锁的节点进行解锁。
...
redis作为一个优秀的开源项目, 它提供了很多功能, 支持用户做更多的功能开发, 所以希望读者能够更加深入的了解redis, 能够更好的使用redis, 能够更好的提升自己的开发能力。